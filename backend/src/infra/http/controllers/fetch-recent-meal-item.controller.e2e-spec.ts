...
describe('Fetch recent meal itens (E2E)', () => {
  ...
  test('[Get] /mealItems', async () => {
    ...
    expect(response.body).toEqual({
      mealItens: expect.arrayContaining([
        expect.objectContaining({ quantityConsumed: 10.0 }),
        expect.objectContaining({ quantityConsumed: 20.0 }),
      ]),
    });
  });
});
