  ...
  test('[Delete] /wastes/:id', async () => {
    ...
    const waste = await db.waste.create({
      data: PrismaWasteMapper.toPrisma(
        makeWaste({ mealItemId: new UniqueEntityID(mealItem.id) }),
      ),
    });

    const response = await request(app.getHttpServer())
      .delete(`/wastes/${waste.id}`)
      .set('Authorization', `Bearer ${accessToken}`)
      .send();

    expect(response.statusCode).toBe(204);

    const wasteOnDatabase = await db.waste.findUnique({
      where: {
        id: waste.id,
      },
    });

    expect(wasteOnDatabase).toBeNull();
  });
});
